...
const BodyContent = ({ Content_State }) => {
    return Content_State.map(list => {
        return (
            <BodyContentMainDivBox key={list.indexs}>
                <div>
                    ...
                    <div className="Writer_Body_Content">{list.content}</div>
                    <div className="Review_Body_Content">
                        <input type="text" value={''} placeholder="댓글을 작성 바랍니다." onChange={() => {}}></input>
                        <button>등록</button>
                    </div>
                </div>
