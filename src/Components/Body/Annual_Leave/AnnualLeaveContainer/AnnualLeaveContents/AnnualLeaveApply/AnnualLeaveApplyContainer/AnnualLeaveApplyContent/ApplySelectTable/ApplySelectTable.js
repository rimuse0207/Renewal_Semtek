...
import {
    Vacation_Calendar_Data_Getting_Redux_Thunk,
    Vacation_Menu_Select_Checked,
} from '../../../../../../../../../Models/Redux-Thunk/VacataionCalendarReduce';
...
const ApplySelectTable = () => {
    ...
    const handleMinusCalendar = () => {
        setGetData(GetData.clone().subtract(2, 'day'));
    };
    const handlePlusCalendar = () => {
        setGetData(GetData.clone().add(2, 'day'));
    };

    useEffect(() => {
        dispatch(Vacation_Calendar_Data_Getting_Redux_Thunk(GetData, Login_Info.id));
    }, [GetData]);
    ...
    const Handle_Selected_Menu_Open = (e, Selected, Selected_Data) => {
        ...
        if (Selected === 'Morning') {
            const Menu_Selected_Checked = Vacation_Date_Data.map(list =>
                list.dateFormat === Selected_Data.dateFormat
                    ? { ...list, Morinig_selected_menu_isChecking: true, Afternoon_selected_menu_isChecking: false }
                    : { ...list, Morinig_selected_menu_isChecking: false, Afternoon_selected_menu_isChecking: false }
            );
            ...
            const Menu_Selected_Checked = Vacation_Date_Data.map(list =>
                list.dateFormat === Selected_Data.dateFormat
                    ? { ...list, Afternoon_selected_menu_isChecking: true, Morinig_selected_menu_isChecking: false }
                    : { ...list, Morinig_selected_menu_isChecking: false, Afternoon_selected_menu_isChecking: false }
            );
            ...
            const Menu_Selected_Checked = Vacation_Date_Data.map(list =>
                list ? { ...list, Morinig_selected_menu_isChecking: false, Afternoon_selected_menu_isChecking: false } : {}
            );
    ...
    const End_Date_Setting_Func = async (e, Selected, Selected_Data) => {
        e.stopPropagation();
        if (Selected === 'Morning') {
            ///종료날짜가 시작 날짜보다 적을 때
            if (moment(clickedDateData.Start_Date).isAfter(moment(Selected_Data.dateFormat), 'day')) {
                ...
                await Vacation_Calendar_State_Change_Fun(Change_Data);
    ...
    const Start_Date_Setting_Func = async (e, Selected, Selected_Data) => {
        e.stopPropagation();
        if (Selected === 'Morning') {
    ...
    const Vacation_Calendar_State_Change_Fun = async Change_Data => {
        const daysBetween = [];
        let currentDate = moment(moment(Change_Data.Start_Date).format('YYYY-MM-DD')).clone();

        while (currentDate.isSameOrBefore(moment(moment(Change_Data.End_Date).format('YYYY-MM-DD')))) {
        ...
        const Menu_Selected_Checked = Vacation_Date_Data.map(list => {
            if (daysBetween.includes(list.dateFormat)) {
                ...
                if (moment(list.dateFormat).isSame(moment(Change_Data.End_Date).format('YYYY-MM-DD'))) {
                    ...
                    if (list.holiday_Check || list.Morning_disabled || list.Afternoon_disabled) {
                        //공휴일 일때는 숫자만 체크
                        if (Change_Data.End_Time === '14:00') {
                            //오전 반차 인경우 오후 해제처리
                            return {
                                ...list,
                                Afternoon_selected_menu_isChecking: false,
                                Morinig_selected_menu_isChecking: false,
                                Morning_clickCheck: false,
                                Morning_Count: 0.5,
                                Afternoon_clickCheck: false,
                                Afternoon_Count: 0,
                            };
                        } else {
                            return {
                ...
                } else if (list.holiday_Check || list.Morning_disabled || list.Afternoon_disabled) {
                    //공휴일  일때는 숫자만 카운트
                    return {
                        ...list,
                        Afternoon_selected_menu_isChecking: false,
        ...
        dispatch(Vacation_Menu_Select_Checked(Menu_Selected_Checked));
        const Weekend_Days_Count = await Weekend_Days_Count_Func(Menu_Selected_Checked);
        const Selected_Days_Count = await Selected_Days_Count_Func(Menu_Selected_Checked);
        const Already_Days_Count = await Already_Days_Count_Func(Menu_Selected_Checked);
        await Selected_ClickedChange_Data(
            moment(Change_Data.Start_Date),
            Change_Data.Start_Time,
            moment(Change_Data.End_Date),
            Change_Data.End_Time,
            Weekend_Days_Count,
            Selected_Days_Count,
            Already_Days_Count
        );
    ...
    //날짜 변경에 따른 날짜 리덕스 변경
    const Selected_ClickedChange_Data = async (Start_Date, Start_Time, End_Date, End_Time, Weekend_days, Select_Days, Already_Days) => {
        const Change_Data = {
            ...clickedDateData,
        ...
        return dispatch(Vacation_Apply_State_Func(Change_Data));
    };

    // 공휴일 갯수 확인
    const Weekend_Days_Count_Func = async Menu_Selected_Checked => {
    ...
    // 연차 사용 확인
    const Selected_Days_Count_Func = async Menu_Selected_Checked => {
        return Menu_Selected_Checked.reduce((accumulator, currentValue) => {
            if (!currentValue.holiday_Check) {
                if (!currentValue.Morning_disabled || !currentValue.Afternoon_disabled) {
                    return accumulator + currentValue.Morning_Count + currentValue.Afternoon_Count;
                } else {
                    return accumulator;
                }
    ...
    // 이미 사용 된 연차 갯수 확인
    const Already_Days_Count_Func = async Menu_Selected_Checked => {
        return Menu_Selected_Checked.reduce((accumulator, currentValue) => {
            if (currentValue.Morning_disabled || currentValue.Afternoon_disabled) {
                return accumulator + currentValue.Morning_Count + currentValue.Afternoon_Count;
            } else {
    ...
    // 시작날짜 변경 시
    const handleStartClickDates = async Select_Dates => {
        const Change_Data = {
            ...clickedDateData,
            Start_Date: moment(Select_Dates).format('YYYY-MM-DD'),
        };
        await Vacation_Calendar_State_Change_Fun(Change_Data);
    };
    //시작 시간 변경 시
    const handleChangeStartTime = async Select_Time => {
        const Change_Data = {
            ...clickedDateData,
            Start_Time: Select_Time.target.value,
        };
        await Vacation_Calendar_State_Change_Fun(Change_Data);
    };

    // 종료날짜 변경 시
    const handleEndClickDates = async Select_Dates => {
        const Change_Data = {
            ...clickedDateData,
            End_Date: moment(Select_Dates).format('YYYY-MM-DD'),
        };
        await Vacation_Calendar_State_Change_Fun(Change_Data);
    };
    // 종료시간 변경 시
    const handleChangeEndTime = async Select_Time => {
        const Change_Data = {
            ...clickedDateData,
            End_Time: Select_Time.target.value,
        };
        await Vacation_Calendar_State_Change_Fun(Change_Data);
    };
                                        ...
                                        <td>
                                            <div className="SelectedCehcking_Container">
                                                {list.holiday_Check || list.Morning_disabled ? (
                                                    <div
                                                        style={{ background: 'gray', height: '100%', opacity: '0.1' }}
                                                        onClick={e => Handle_Selected_Menu_Open(e, 'Disabled')}
                                                    ></div>
                                                ) : list.Morning_clickCheck ? (
                                                    <div
                                                        className="Now_Checked"
                                                        onClick={e => Handle_Selected_Menu_Open(e, 'Morning', list)}
                                                    >
                                                        {list.Morinig_selected_menu_isChecking ? (
                                                            <ul className="Select_Menu">
                                                                <li onClick={e => Start_Date_Setting_Func(e, 'Morning', list)}>
                                                                    시작날짜로 선택
                                                                </li>
                                                                <li onClick={e => End_Date_Setting_Func(e, 'Morning', list)}>
                                                                    종료날짜로 선택
                                                                </li>
                                                            </ul>
                                                        ) : (
                                            ...
                                            <div className="SelectedCehcking_Container">
                                                {list.holiday_Check || list.Afternoon_disabled ? (
                                                    <div
                                                        ...
                                                        onClick={e => Handle_Selected_Menu_Open(e, 'Disabled')}
                                                    ></div>
                                                ) : list.Afternoon_clickCheck ? (
                                                    <div
                                                        className="Now_Checked"
                                                        onClick={e => Handle_Selected_Menu_Open(e, 'Afternoon', list)}
                                                    >
                                                        {list.Afternoon_selected_menu_isChecking ? (
                                                            <ul className="Select_Menu">
                                                                <li onClick={e => Start_Date_Setting_Func(e, 'Afternoon', list)}>
                                                                    시작날짜로 선택
                                                                </li>
                                                                <li onClick={e => End_Date_Setting_Func(e, 'Afternoon', list)}>
                                                                    종료날짜로 선택
                                                                </li>
                                                            </ul>
                                                        ) : (
                                ...
                                <div>
                                    <div className="Date_Pickers_Container">
                                        ...
                                        <div className="Date_Pickers_Pickers">
                                            <DatePicker
                                                selected={new Date(clickedDateData.Start_Date)}
                                                onChange={date => handleStartClickDates(date)}
                                    ...
                                    <div className="Time_Pickers_Container">
                                        <div className="Hour_Pickers_Container">
                                            <select
                                                value={clickedDateData.Start_Time}
                                                onChange={e => {
                                ...
                                <div className="Select_Vacation_Lists">
                                    <div>
                                        <div className="Date_Pickers_Container">
                                            ...
                                            <div className="Date_Pickers_Pickers">
                                                <DatePicker
                                                    selected={new Date(clickedDateData.End_Date)}
                                        ...
                                        <div className="Time_Pickers_Container">
                                            <div className="Hour_Pickers_Container">
                                                <select
                                                    value={clickedDateData.End_Time}
                                                    onChange={e => {
