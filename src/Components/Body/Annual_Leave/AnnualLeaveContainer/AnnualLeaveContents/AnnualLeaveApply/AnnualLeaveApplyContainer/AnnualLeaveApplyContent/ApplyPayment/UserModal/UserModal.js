...
import {
    Payment_Accept_User_Change_Func,
    Payment_Apply_User_Change_Func,
    Payment_Review_User_Change_Func,
} from '../../../../../../../../../../Models/PaymentUserReducer/PaymentUserReducer';
...
const UserModal = ({ modalIsOpen, setModalIsOpen }) => {
    ...
    const [User_Search_Value, setUser_Search_Value] = useState(null);
    ...
    const handleUserClick = data => {
        const ChangeUser = UserDetailInfo.map(list => (list.uid === data.uid ? { ...list, checked: !data.checked } : list));
        setUserDetailInfo(ChangeUser);
    };

    const handleAllChecked = e => {
        setUserDetailInfo(UserDetailInfo.map(list => (e.target.checked ? { ...list, checked: true } : { ...list, checked: false })));
    };
    ...
    return (
        <Modal isOpen={modalIsOpen} style={customStyles} onRequestClose={() => setModalIsOpen()}>
            <UserModalMainDivBox>
                <div className="Main_Container">
                    <div className="First_Container">
                        <OrganChartMainPage
                            ...
                            setUserDetailInfo={data => setUserDetailInfo(data)}
                            setSelectUserInfoData={data => setSelectUserInfoData(data)}
                            setSelectUserNames={data => setSelectUserNames(data)}
                            UserSearchModalOn={modalIsOpen}
                    ...
                    <div className="Second_Container">
                        <div className="SearchNames_container">
                            <Select
                                ...
                                onChange={e => setUser_Search_Value(e)}
                            />
                        </div>
                                        ...
                                        {UserDetailInfo.map((list, j) => {
                                            return (
                                                <tr
                                                    key={list.uid}
                                                    style={
                                                        SelectUserNames === list.uid
                                                            ? { backgroundColor: 'yellow' }
                                                            : list.checked
                                                            ? { backgroundColor: 'aliceblue' }
                                                            : {}
                                                    }
                                                    onClick={() => handleUserClick(list)}
                                                >
                                                    <td>
                                                        <input type="checkbox" checked={list.checked} readOnly></input>
                                                    </td>
                                                    <td>{j + 1}. </td>
                                                    <td>{list.cn}</td>
                    ...
                    <div className="Third_Container">
                        ...
                        <div>
                            ...
                            <div className="ArrowIcons_Container">{/* <TbSquareRoundedArrowLeft></TbSquareRoundedArrowLeft> */}</div>
                        </div>
                    </div>
                    <div className="Fourth_Container">
                        <div className="Selected_Container">
                            ...
                            <div>
                                {PrePare_ApplyUser_State.map((list, i) => {
                                    return (
                                        <ul className="Selected_User_Container">
                                            <li>{i + 1}. </li>
                                            <li>{list.cn}</li>
                                            <li>{list.department_name}</li>
                                            <li onClick={() => handleClickDeletUser(list, 'Apply')}>
                        ...
                        <div className="Selected_Container">
                            ...
                            <div>
                                {' '}
                                {PrePare_ReviewUser_State.map((list, i) => {
                                    return (
                                        <ul className="Selected_User_Container">
                                            <li>{i + 1}. </li>
                                            <li>{list.cn}</li>
                                            <li>{list.department_name}</li>
                                            <li onClick={() => handleClickDeletUser(list, 'Review')}>
                                                <AiOutlineUserDelete></AiOutlineUserDelete>
                                            </li>
                                        </ul>
                                    );
                        ...
                        <div className="Selected_Container">
                            ...
                            <div>
                                {' '}
                                {PrePare_AcceptUser_State.map((list, i) => {
                                    return (
                                        <ul className="Selected_User_Container">
                                            <li>{i + 1}. </li>
                                            <li>{list.cn}</li>
                                            <li>{list.department_name}</li>
                                            <li onClick={() => handleClickDeletUser(list, 'Accept')}>
                                                <AiOutlineUserDelete></AiOutlineUserDelete>
                                            </li>
                                        </ul>
                                    );
