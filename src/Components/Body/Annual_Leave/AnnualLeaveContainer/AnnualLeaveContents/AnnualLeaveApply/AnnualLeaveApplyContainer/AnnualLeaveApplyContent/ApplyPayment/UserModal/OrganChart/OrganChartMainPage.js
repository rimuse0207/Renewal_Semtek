...
const OrganChartMainPage = ({ ClickedUser, setUserDetailInfo, setSelectUserInfoData, setSelectUserNames, UserSearchModalOn }) => {
    ...
    const ChangeSelectedInfoGettingPersonInfo = async () => {
        try {
            const ChangeSelectedInfoGettingPersonInfo_Axios = await request.get(
                `/DepartmentRouter/ChangeSelectedInfoGettingPersonInfo_Semtek`,
                {
                    params: {
                        ClickItemSelected,
                    },
                ...
                {OrganChartState.map(list => {
                    return list.children.length > 0 ? (
                        ...
                        >
                            <OrganChartData
                                items={list.children}
                                HandleClickItemShowInfo={content => HandleClickItemShowInfo(content)}
                            ></OrganChartData>
                        </Tree>
                    ) : (
                        <Tree
